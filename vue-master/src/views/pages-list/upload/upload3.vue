    ...
    <div id="box">
      <div id="upload">
        <div class="img-video">
          <div class="ul">
            <div class="li">
              <van-uploader :accept="accept" v-model="fileList" multiple preview-size="80px" :max-count="maxImageZhang" :before-delete="beforeDelete"></van-uploader>
            </div>
          </div>
        </div>
      </div>
...
export default {
  ...
  methods: {
    ...
    submit() {
      ...
      uploadImg(this.FormData, this.upLoadUrl).then(res => {
        setTimeout(() => {
          this.$toast("上传成功")
