...
export default {
  install(Vue) {
    ...
    Vue.prototype.initilize = function (opt) {
      let self = this;
      this.options = opt;
      if (self.options.clipperInit) self.options.clipperInit()
      //创建dom
      this.createElement();
      ...
      this.cropper = new Cropper(this.preview, {
        ...
        ready: function () {
          ...
          if (self.options.clipperInitOk) self.options.clipperInitOk()
    ...
    Vue.prototype.createElement = function () {
      ...
      // str += '<div class="crop_loading"><div class="crop_content"><img id="clip_loading_gif" src=""><div class="crop_text">图片裁剪中</div></div></div>';
      // str += '<div class="crop_success"><div class="crop_success_text">裁剪完成</div></div></div>';
      ...
      // var img = document.getElementById('clip_loading_gif');
    ...
    Vue.prototype.crop = function () {
      ...
      if (self.options.clipperStart) self.options.clipperStart()
      setTimeout(function () {
    ...
    Vue.prototype.destoried = function () {
      ...
      //移除裁剪框
      if (this.reagion) this.reagion.parentNode.removeChild(this.reagion);
    ...
    Vue.prototype.postImg = function (imageData) {
      ...
      window.setTimeout(function () {
        ...
        //裁剪完后摧毁对象
        self.destoried();
        if (self.options.clipperOk) self.options.clipperOk({ imgUrl: imageData })
      }, 3000)
    }
