...
import { getQueryString, getIsWxClient, getBrowserEnvironment, loadScript } from 'assets/js/util'
...
function _getPageConfig(config) {
  if (!PROJECT_CONFIG_CODE) {
    ...
    getProjectConfig().then(res => {
      ...
      SHARECONFIG.ShareUrl = _data.project_url || AUTH_URL
...
async function _openDebugging(onlineDate, offlinedate) {
  ...
  if (PROJECT_CONFIG.vConsole.is_open == 1 || (PROJECT_CONFIG.vConsole.is_open == 2 && isWx) || (PROJECT_CONFIG.vConsole.is_open == 3 && isWx && _is_go_online)) {
