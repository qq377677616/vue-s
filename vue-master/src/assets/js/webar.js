...
export default class WebAR {
            ...
            navigator.mediaDevices.enumerateDevices().then((devices) => {
                ...
                devices.find((device) => {
                    if (device.kind === 'videoinput') {
                        // 在iOS12.2上deviceId为空
                        if (device.deviceId == '') {
                          videoDeviceList.push({ audio: false, video: { facingMode: { exact: this.cameras[0] } } })
                          videoDeviceList.push({ audio: false, video: { facingMode: { exact: this.cameras[1] } } })
                        } else {
                          videoDeviceList.push({ audio: false, video: { deviceId: { exact: device.deviceId } } })
                        }
                ...
                if (videoDeviceList.length === 0) {
                    console.log("【没有可以使用的视频设备】")
                    reject('没有可使用的视频设备');
                } else {
                    console.log("【有可以使用的视频设备】")
                    this.initVideo();
                    this.initCanvas();
     ...
     * @returns {string}
     */
    captureVideo(cabllback) {
        console.log("this.videoElement.offsetWidth, this.videoElement.offsetHeight", this.videoElement.offsetWidth, this.videoElement.offsetHeight)
        this.canvasContext.drawImage(this.videoElement, 0, 0, this.videoElement.offsetWidth, this.videoElement.offsetHeight);
        let _img = this.canvasElement.toDataURL('image/jpeg', 1)
        // cabllback(this.canvasElement.toDataURL('image/jpeg', 1))
        return new Promise((resolve, reject) => {
          resolve(_img)    
        })
    }
    // captureVideo() {
    //     this.canvasContext.drawImage(this.videoElement, 0, 0, this.videoElement.offsetWidth, this.videoElement.offsetHeight);
    //     return { image: this.canvasElement.toDataURL('image/jpeg', 0.5).split('base64,')[1], img: this.canvasElement.toDataURL('image/jpeg', 0.5) };
    // }
    /**
     * 创建视频详情元素，播放摄像头视频流
     */
    initVideo() {
        ...
        this.videoElement.setAttribute('id', 'arvideo');
        this.videoElement.style = 'position: fixed;left:0;top:0;z-index: 99;object-fit:cover;';
        console.log("【创建的video】", this.videoElement)
    ...
    startRecognize(callback) {
        // this.timer = window.setInterval(() => {
            ...
            const image = this.captureVideo()
            document.getElementById("photograph").src = image.img
